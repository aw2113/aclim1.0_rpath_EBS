...
gfd85.f1.91.add <- run.hind91
# rsim.plot(gfd85.f1.91.add, bal$Group, indplot = F)
...
for (yr in fore_years){
  ...
  F_ABC <- Ftarget 
  C_ABC <- F_ABC*end_biomass(gfd85.f1.91.add)
  
  # Institute control rule for crab species
  sp <- crab_sp
  falpha     <- 0.05
  fbeta      <- 0.20
  assessment <- end_biomass(gfd85.f1.91.add) #* rnorm(length(end_biomass(gfd85.f1.91.add)), mean=1, sd=0.2)
  ...
  # Run an rsim year
  gfd85.f1.91.add <- rsim.step(scene.full91,gfd85.f1.91.add,method='AB',year.end=yr)
}
#END OF RUN#####################################################################

rsim.plotly(gfd85.f1.91.add,capped_sp)
rsim.plot(gfd85.f1.91.add,"")
...
par(mfrow=c(3,3))
for (i in 2:10) {
  plot(gfd85.f1.91.add$annual_Biomass[,i], type='l', col='deepskyblue', ylim=c(0,max(gfd85.f1.91.add$annual_Biomass[,i])), lwd=2, main=c(bal$Group[i-1], "gfd85"),
       ylab="Annual Biomass")
  # lines(gfd85.f1.91.pro$annual_Biomass[,i], type = 'l', col='red', lwd=2)
  lines(gfd85.f1.91.add$annual_Biomass[1:27,i], col="gray50", lwd=2)
...
par(mfrow=c(3,3))
for (i in 2:10) {
  plot(gfd85.f1.91.add$annual_Catch[,i], type='l', col='red', ylim=c(0,max(gfd85.f1.91.add$annual_Catch[,i])), lwd=2, main=c(bal$Group[i-1], "gfd85"),
       ylab="Catch")
  # lines(gfd85.f1.91.pro$annual_Biomass[,i], type = 'l', col='red', lwd=2)
  lines(gfd85.f1.91.add$annual_Catch[1:27,i], col="gray50", lwd=2)
  abline(v=27, lwd=2, lty=2)
  abline(h=mean(gfd85.f1.91.add$annual_Catch[1:27,i]), col="black")
}

# Seabirds
par(mfrow=c(2,3))
for (i in 11:16) {
  plot(gfd85.f1.91.add$annual_Catch[,i], type='l', col='deepskyblue', ylim=c(0,max(gfd85.f1.91.add$annual_Catch[,i])), lwd=2, main=c(bal$Group[i-1], "gfd85"),
       ylab="Catch")
  # lines(gfd85.f1.91.pro$annual_Biomass[,i], type = 'l', col='red', lwd=2)
  lines(gfd85.f1.91.add$annual_Catch[1:27,i], col="gray50", lwd=2)
  abline(v=27, lwd=2, lty=2)
...
for (i in 17:36) {
  plot(gfd85.f1.91.add$annual_Catch[,i], type='l', col='deepskyblue', ylim=c(0,max(gfd85.f1.91.add$annual_Catch[,i])), lwd=2, main=c(bal$Group[i-1], "gfd85"),
       ylab="Catch")
  # lines(gfd85.f1.91.pro$annual_Catch[,i], type = 'l', col='red', lwd=2)
  lines(gfd85.f1.91.add$annual_Catch[1:27,i], col="gray50", lwd=2)
  abline(v=27, lwd=2, lty=2)
  abline(h=mean(gfd85.f1.91.add$annual_Catch[1:27,i]), col="black")
}
