...
#' This the model developed for ACLIM and IFRAME. There are 72 biological groups
#' (including detritus). This aggregation scheme includes stanzas for POL, COD,
#' HAL, ATF, and GT, and N fur seals.
#' 
#' -----------------***** UPDATE *****---------------------------------------- #
#' Updating the script to examine the differences between using the addivtive vs.
#' the proportional method of bias-correction. Each method has its caveats. Also,
#' updating the forecast forcing by changing the anomaly multiplier to the BaseB
#' biomass.
#' 
#' -----------------***** UPDATE *****---------------------------------------- #
#' October 2, 2019
#' 
#' Re-running all sims with updated hindcast and CMIP5 projections.
#' The updated hindcast runs through 2017. Historical catch has also been updated
#' and now runs through 2017.
#' 
#' -----------------***** UPDATE *****---------------------------------------- #
#' February 22, 2021
...
# GFDL_rcp45
gfd45_add <- read.csv("climate/gfd45_add.csv")
UU  <- TT+1
# Set groups to force
...
scene.full91$forcing$ForcedBio[UU:1308,"Euphausiids"]      <- BaseB["Euphausiids"]      *(1.0+gfd45_add[145:1128,"EUP_gfd45"])
scene.full91$forcing$ForcedBio[UU:1308,"Copepods"]         <- BaseB["Copepods"]         *(1.0+gfd45_add[145:1128,"COP_gfd45"])
scene.full91$forcing$ForcedBio[UU:1308,"Pelagic.microbes"] <- BaseB["Pelagic.microbes"] *(1.0+gfd45_add[145:1128,"MZP_gfd45"])
scene.full91$forcing$ForcedBio[UU:1308,"Benthic.microbes"] <- BaseB["Benthic.microbes"] *(1.0+gfd45_add[145:1128,"MZP_gfd45"])
scene.full91$forcing$ForcedBio[UU:1308,"Lg.phytoplankton"] <- BaseB["Lg.phytoplankton"] *(1.0+gfd45_add[145:1128,"PhL_gfd45"])
scene.full91$forcing$ForcedBio[UU:1308,"Sm.phytoplankton"] <- BaseB["Sm.phytoplankton"] *(1.0+gfd45_add[145:1128,"PhS_gfd45"])   
# ------------------------------------------------------------------------ #

# Run hindcast years
run.hind91 <- rsim.run(scene.full91,method='AB',years=hind_years91)
gfd45.f1.91.add <- run.hind91
# rsim.plot(gfd45.f1.91.add, bal$Group, indplot = F)
...
for (yr in fore_years){
  ...
  F_ABC <- Ftarget 
  C_ABC <- F_ABC*end_biomass(gfd45.f1.91.add)
  
  # Institute control rule for crab species
  sp <- crab_sp
  falpha     <- 0.05
  fbeta      <- 0.20
  assessment <- end_biomass(gfd45.f1.91.add) #* rnorm(length(end_biomass(gfd45.f1.91.add)), mean=1, sd=0.2)
  ...
  # Run an rsim year
  gfd45.f1.91.add <- rsim.step(scene.full91,gfd45.f1.91.add,method='AB',year.end=yr)
}
#END OF RUN#####################################################################

rsim.plotly(gfd45.f1.91.add,capped_sp)
rsim.plot(gfd45.f1.91.add,"")
abline(v=324, lwd=2, lty=2)

################################################################################
################################################################################
################################################################################
# plot as annual biomass

# Mammals
par(mfrow=c(3,3))
for (i in 2:10) {
  plot(gfd45.f1.91.add$annual_Biomass[,i], type='l', col='deepskyblue', ylim=c(0,max(gfd45.f1.91.add$annual_Biomass[,i])), lwd=2, main=c(bal$Group[i-1], "gfd45"),
       ylab="Annual Biomass")
  # lines(gfd45.f1.91.pro$annual_Biomass[,i], type = 'l', col='red', lwd=2)
  lines(gfd45.f1.91.add$annual_Biomass[1:27,i], col="gray50", lwd=2)
...
par(mfrow=c(3,3))
for (i in 2:10) {
  plot(gfd45.f1.91.add$annual_Catch[,i], type='l', col='red', ylim=c(0,max(gfd45.f1.91.add$annual_Catch[,i])), lwd=2, main=c(bal$Group[i-1], "gfd45"),
       ylab="Catch")
  # lines(gfd45.f1.91.pro$annual_Biomass[,i], type = 'l', col='red', lwd=2)
  lines(gfd45.f1.91.add$annual_Catch[1:27,i], col="gray50", lwd=2)
  abline(v=27, lwd=2, lty=2)
  abline(h=mean(gfd45.f1.91.add$annual_Catch[1:27,i]), col="black")
}

# Seabirds
par(mfrow=c(2,3))
for (i in 11:16) {
  plot(gfd45.f1.91.add$annual_Catch[,i], type='l', col='deepskyblue', ylim=c(0,max(gfd45.f1.91.add$annual_Catch[,i])), lwd=2, main=c(bal$Group[i-1], "gfd45"),
       ylab="Catch")
  # lines(gfd45.f1.91.pro$annual_Biomass[,i], type = 'l', col='red', lwd=2)
  lines(gfd45.f1.91.add$annual_Catch[1:27,i], col="gray50", lwd=2)
  abline(v=27, lwd=2, lty=2)
...
for (i in 17:36) {
  plot(gfd45.f1.91.add$annual_Catch[,i], type='l', col='deepskyblue', ylim=c(0,max(gfd45.f1.91.add$annual_Catch[,i])), lwd=2, main=c(bal$Group[i-1], "gfd45"),
       ylab="Catch")
  # lines(gfd45.f1.91.pro$annual_Catch[,i], type = 'l', col='red', lwd=2)
  lines(gfd45.f1.91.add$annual_Catch[1:27,i], col="gray50", lwd=2)
  abline(v=27, lwd=2, lty=2)
  abline(h=mean(gfd45.f1.91.add$annual_Catch[1:27,i]), col="black")
}
