...
ces85.f3.91.add <- run.hind91
# rsim.plot(ces85.f3.91.add, bal$Group, indplot = F)
...
for (yr in fore_years){
  ...
  F_ABC <- Ftarget 
  C_ABC <- F_ABC*end_biomass(ces85.f3.91.add)
  
  # Institute control rule for crab species
  sp <- crab_sp
  falpha     <- 0.05
  fbeta      <- 0.20
  assessment <- end_biomass(ces85.f3.91.add) #* rnorm(length(end_biomass(ces85.f3.91.add)), mean=1, sd=0.2)
  ...
  # Run an rsim year
  ces85.f3.91.add <- rsim.step(scene.full91,ces85.f3.91.add,method='AB',year.end=yr)
}
#END OF RUN#####################################################################

# rsim.plotly(ces85.f3.91.add,capped_sp)
rsim.plot(ces85.f3.91.add,"")
