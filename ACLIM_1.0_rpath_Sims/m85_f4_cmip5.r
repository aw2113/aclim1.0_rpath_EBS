...
# MIROC_rcp85
mir85_add <- read.csv("climate/mir85_add.csv")
UU  <- TT+1
# Set groups to force
...
scene.full91$forcing$ForcedBio[UU:1308,"Euphausiids"]      <- BaseB["Euphausiids"]      *(1.0+mir85_add[145:1128,"EUP_mir85"])
scene.full91$forcing$ForcedBio[UU:1308,"Copepods"]         <- BaseB["Copepods"]         *(1.0+mir85_add[145:1128,"COP_mir85"])
scene.full91$forcing$ForcedBio[UU:1308,"Pelagic.microbes"] <- BaseB["Pelagic.microbes"] *(1.0+mir85_add[145:1128,"MZP_mir85"])
scene.full91$forcing$ForcedBio[UU:1308,"Benthic.microbes"] <- BaseB["Benthic.microbes"] *(1.0+mir85_add[145:1128,"MZP_mir85"])
scene.full91$forcing$ForcedBio[UU:1308,"Lg.phytoplankton"] <- BaseB["Lg.phytoplankton"] *(1.0+mir85_add[145:1128,"PhL_mir85"])
scene.full91$forcing$ForcedBio[UU:1308,"Sm.phytoplankton"] <- BaseB["Sm.phytoplankton"] *(1.0+mir85_add[145:1128,"PhS_mir85"])   
# ------------------------------------------------------------------------ #

# Run hindcast years
run.hind91 <- rsim.run(scene.full91,method='AB',years=hind_years91)
mir85.f4.91.add <- run.hind91
# rsim.plot(mir85.f4.91.add, bal$Group, indplot = F)
...
for (yr in fore_years){
  ...
  F_ABC <- Ftarget 
  C_ABC <- F_ABC*end_biomass(mir85.f4.91.add)
  
  # Institute control rule for crab species
  sp <- crab_sp
  falpha     <- 0.05
  fbeta      <- 0.20
  assessment <- end_biomass(mir85.f4.91.add) #* rnorm(length(end_biomass(mir85.f4.91.add)), mean=1, sd=0.2)
  ...
  # Run an rsim year
  mir85.f4.91.add <- rsim.step(scene.full91,mir85.f4.91.add,method='AB',year.end=yr)
}
#END OF RUN#####################################################################

# rsim.plotly(mir85.f4.91.add,capped_sp)
rsim.plot(mir85.f4.91.add,"")
