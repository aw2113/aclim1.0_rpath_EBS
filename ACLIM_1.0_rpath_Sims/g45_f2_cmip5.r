...
gfd45.f2.91.add <- run.hind91
# rsim.plot(gfd45.f2.91.add, bal$Group, indplot = F)
...
for (yr in fore_years){
  ...
  F_ABC <- Ftarget 
  C_ABC <- F_ABC*end_biomass(gfd45.f2.91.add)
  
  # Institute control rule for crab species
  sp <- crab_sp
  falpha     <- 0.05
  fbeta      <- 0.20
  assessment <- end_biomass(gfd45.f2.91.add) #* rnorm(length(end_biomass(gfd45.f2.91.add)), mean=1, sd=0.2)
  ...
  # Run an rsim year
  gfd45.f2.91.add <- rsim.step(scene.full91,gfd45.f2.91.add,method='AB',year.end=yr)
}
#END OF RUN#####################################################################

# rsim.plotly(gfd45.f2.91.add,capped_sp)
rsim.plot(gfd45.f2.91.add,"")
