...
# CESM_rcp45
ces45_add <- read.csv("climate/ces45_add.csv")
UU  <- TT+1
# Set groups to force
...
scene.full91$forcing$ForcedBio[UU:1080,"Euphausiids"]      <- BaseB["Euphausiids"]      *(1.0+ces45_add[145:900,"EUP_ces45"])
scene.full91$forcing$ForcedBio[UU:1080,"Copepods"]         <- BaseB["Copepods"]         *(1.0+ces45_add[145:900,"COP_ces45"])
scene.full91$forcing$ForcedBio[UU:1080,"Pelagic.microbes"] <- BaseB["Pelagic.microbes"] *(1.0+ces45_add[145:900,"MZP_ces45"])
scene.full91$forcing$ForcedBio[UU:1080,"Benthic.microbes"] <- BaseB["Benthic.microbes"] *(1.0+ces45_add[145:900,"MZP_ces45"])
scene.full91$forcing$ForcedBio[UU:1080,"Lg.phytoplankton"] <- BaseB["Lg.phytoplankton"] *(1.0+ces45_add[145:900,"PhL_ces45"])
scene.full91$forcing$ForcedBio[UU:1080,"Sm.phytoplankton"] <- BaseB["Sm.phytoplankton"] *(1.0+ces45_add[145:900,"PhS_ces45"])   
# ------------------------------------------------------------------------ #

# Run hindcast years
run.hind91 <- rsim.run(scene.full91,method='AB',years=hind_years91)
ces45.f1.91.add <- run.hind91
# rsim.plot(ces45.f1.91.add, bal$Group, indplot = F)
...
for (yr in fore_years){
  ...
  F_ABC <- Ftarget 
  C_ABC <- F_ABC*end_biomass(ces45.f1.91.add)
  
  # Institute control rule for crab species
  sp <- crab_sp
  falpha     <- 0.05
  fbeta      <- 0.20
  assessment <- end_biomass(ces45.f1.91.add) #* rnorm(length(end_biomass(ces45.f1.91.add)), mean=1, sd=0.2)
  ...
  # Run an rsim year
  ces45.f1.91.add <- rsim.step(scene.full91,ces45.f1.91.add,method='AB',year.end=yr)
}
#END OF RUN#####################################################################

# rsim.plotly(ces45.f1.91.add,capped_sp)
rsim.plot(ces45.f1.91.add,"")
